...
export async function POST(request: Request) {
  try {
    ...
    const { roomName, identity, destinationRoom } = req;

    if (!roomName || !identity || !destinationRoom) {
      return new NextResponse("Missing roomName, identity or destinationRoom", { status: 400 });
    }
